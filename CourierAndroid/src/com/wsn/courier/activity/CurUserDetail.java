...
import com.wsn.courier.ChangeAddressActivity;
import com.wsn.courier.ChangePhoneActivity;
...
public class CurUserDetail extends Activity {
	LinearLayout ll_cur_gender,ll_cur_phone,ll_cur_address;//点击
	LinearLayout ll_user_phone,ll_user_address;//存放电话、地址
	TextView tv_cur_name,tv_cur_login,tv_cur_gender,tv_cur_phone,tv_cur_address;
	private float scale;
	userVo vo;
	@Override
	...
	private void init() {
		...
		vo = Session.getCurrUserVO();
		tv_cur_name.setText(vo.getName());
		tv_cur_login.setText(vo.getLoginName());
		if("true".equalsIgnoreCase(vo.getGender())){
			tv_cur_gender.setText("男");
		}else tv_cur_gender.setText("女");
		setPhone();
		setAddress();
