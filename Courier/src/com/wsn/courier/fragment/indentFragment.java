...
//	String url = "http://10.128.19.233:8080/delivery/";
	...
	List<indentVo> sendedList = null;
	List<indentVo> receivedList = null;
	boolean sendedProgress = false,receivedProgress = false;
	Handler handler;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		System.out.println("运行indentFragment的onCreateView+++++++++++++++++++++++++++++++++++");
		super.onCreateView(inflater, container, savedInstanceState);
		View indentView = inflater.inflate(R.layout.fragment_chat, container,false);
		getData();
	...
	private void getData() {
		...
		String URL1 = getResources().getString(R.string.url)+"getSendedIndent?id="+Session.getCurrUserVO().getUserId(); 
		...
		String URL2 = getResources().getString(R.string.url)+"getReceivedIndent?id="+Session.getCurrUserVO().getUserId(); 
		Request<JSONArray> request_Received = new JsonArrayRequest(URL2, new Listener<JSONArray>() {
			@Override
			public void onResponse(JSONArray response) {
				addReceivedData(response);
				System.out.println("receivedData:"+receivedList.size()+">>>>");
				Message msg = new Message();
			    msg.what = 2;
			    handler.sendMessage(msg);
			}
		...
		rq.add(request_Received);
	}

	private void addReceivedData(JSONArray response) {
		receivedList = new ArrayList<indentVo>();
